...
Текущий стабильный релиз — **1.5.1**, предрелиз — **1.5.2-rc.3**. Публичный Rust extension API v6 и IPC
...
Daemon создаёт или дополняет `%LOCALAPPDATA%\VoiceAssistantCore\logs\assistant-daemon.log`.
Журнал содержит предупреждения и ошибки всех компонентов, внутренние диагностические
события ядра, исходные цепочки ошибок, место в коде, поток и backtrace паники. Аудио
и полный распознанный текст в журнал не записываются. Обычное закрытие IPC-клиента
после запроса не считается ошибкой и в журнал не попадает.
...
На Windows скачанные blobs копируются напрямую без snapshot symlinks, поэтому установка
не зависит от режима разработчика. Сбой или panic источника модели возвращается как
`ModelError::Hub`, не завершая background task.
